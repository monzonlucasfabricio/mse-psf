...
2)  Espectro en frecuencia de la señal.
3)  A simple vista se distinguen las frecuencias de 50Hz y 48 Hz.
...
xFFT.set_ylim(np.min(np.abs(fft_result)),50)
...
xFFTZero.set_ylim(np.min(np.abs(fft_result)),50)
...
xLnFFTZero.set_data(freqs, np.abs(fft_result))
